...
// --- unified rAF-smoothed scroll effects (parallax + horizontal carousel) ---
  ...
  // scroll window so the pinned programs carousel lands on card i (mega-menu anchors)
  window.scrollToProgram = function (i) {
    const wrap = document.getElementById('programi-wrap');
    const sec = document.getElementById('programi-sec');
    const track = sec && sec.querySelector('[x-ref="track"]');
    if (!wrap || !sec || !track) return;
    const dist = Math.max(track.scrollWidth - track.clientWidth, 0);
    const wrapTop = wrap.getBoundingClientRect().top + window.scrollY;
    const topOffset = Math.min(0, window.innerHeight - sec.offsetHeight);
    let y = wrapTop - topOffset;
    const card = track.querySelectorAll('article')[i];
    if (card && dist) {
      const left = card.getBoundingClientRect().left - track.getBoundingClientRect().left + track.scrollLeft - 24;
      y += Math.min(Math.max(left, 0), dist);
    }
